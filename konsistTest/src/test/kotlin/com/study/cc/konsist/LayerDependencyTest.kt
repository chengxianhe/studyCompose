...
class LayerDependencyTest {
    ...
    private val allFiles = Konsist.scopeFromProject().files
        .filterNot { it.path.contains("/konsistTest/") }

    @Test
    fun `domain 模块不依赖任何其他模块或 Android`() {
        val violations = allFiles
    ...
    fun `domain 包禁止出现 android 或 androidx import`() {
        val violations = allFiles
            .filter { it.path.contains("/domain/") }
            .filter { file -> file.imports.any { it.name.startsWith("android.") || it.name.startsWith("androidx.") } }
        assertTrue(
